...
const latitude = 27.702946948951418;
const longitude = 85.32275941926572;
...
function setupMap(coords) {
    ...
    map.on('load', () => {
        ...
        map.loadImage('img/file (26).png', (error, image) => {
...
const createDots = function () {
    slides.forEach(function (slide, index) {
        dotContainer.insertAdjacentHTML(
            `beforeend`,
            `<button  class="dots--dot" data-slide="${index}" aria-label="photo-${index + 1
            }"></button>`
        );
    });
};
